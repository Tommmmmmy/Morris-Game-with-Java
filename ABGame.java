...
public class ABGame {
	static int sum = 0;
	static int[] count;
	public static boardPosition midEnd(String position, int depth, char player, int alpha, int beta){
		...
		for(String board : positionList){
			count[depth - 1]++;
			if(player == 'W'){
				boardPosition next = midEnd(board, depth - 1, 'B', alpha, beta);
				...
				boardPosition next = midEnd(board, depth - 1, 'W', alpha, beta);
//				System.out.println(depth + ": " + "'" + res.getAlpha() + "'" + " " + next.value + " " + "'" + res.getBeta() + "'");
	...
	public static void main(String[] args) throws IOException {
        ...
        boardPosition bp = midEnd(opening, depth, 'W', Integer.MIN_VALUE, Integer.MAX_VALUE);
